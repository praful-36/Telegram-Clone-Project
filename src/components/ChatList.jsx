    ...
    <div className="w-full sm:w-1/3 lg:w-1/3 overflow-y-auto">
      <div className="flex items-center">
        ...
        <div className="p-4 w-full">
          <input
            ...
            className="p-1 border rounded-md w-full dark:bg-gray-700 dark:text-white "
          />
        </div>
      </div>
      {filteredChats.length === 0 ? (
        ...
        filteredChats.map(chat => (
          <div
            key={chat.id}
            className={`p-4 border-b dark:border-gray-700 cursor-pointer ${selectedChat && selectedChat.id === chat.id ? 'bg-blue-200' : ''}`}
